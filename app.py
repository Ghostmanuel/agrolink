...
def ensure_columns():
    needed = {
      ...
      "payments": [("gateway_reference","VARCHAR(120)")]
    }
    insp=inspect(engine)
...
app=FastAPI(title="AgroLink Angola API", version="5.0.0",
...
def user_json(u):
    return {"id":u.id,"name":u.name,"phone":u.phone,"role":u.role,"province":u.province,"verified":u.verified,
...
class AssignDriver(BaseModel): driver_id:int
class VehicleIn(BaseModel):
    plate:str; model:str=""; vehicle_type:str="camião"; capacity_kg:float=0; photo_data:Optional[str]=None
class ConversationIn(BaseModel):
    seller_id:int; product_id:Optional[int]=None; order_id:Optional[int]=None; driver_id:Optional[int]=None
class MessageIn(BaseModel): content:str; proposed_price:Optional[float]=None
class ReviewIn(BaseModel):
    delivery_address:Optional[str]=None; confirm:bool
class PaymentIn(BaseModel): order_id:int; method:str

@app.get("/health")
def health(): return {"status":"ok","app":"AgroLink Angola","version":"5.0.0","commission_rate":COMMISSION_RATE}
...
def register(x:Register,s:Session=Depends(db)):
    if s.scalar(select(User).where(User.phone==x.phone)): raise HTTPException(409,"Telefone já registado.")
    if x.role not in {"buyer","farmer","driver"}: raise HTTPException(400,"Perfil inválido.")
    u=User(name=x.name,phone=x.phone,password_hash=pwd.hash(x.password),role=x.role,province=x.province,
           iban=x.iban,express_phone=x.express_phone,company_name=x.company_name,address=x.address,photo_data=x.photo_data)
...
def login(x:Login,s:Session=Depends(db)):
    ...
    if not u or not pwd.verify(x.password,u.password_hash): raise HTTPException(401,"Credenciais inválidas.")
    return {"access_token":token_for(u),"user":user_json(u)}
...
def reset_password(phone:str,code:str,new_password:str,s:Session=Depends(db)):
    if len(new_password)<6: raise HTTPException(400,"A nova senha deve ter pelo menos 6 caracteres.")
    u=s.scalar(select(User).where(User.phone==phone))
    ...
    u.password_hash=pwd.hash(new_password); r.used=True
...
def update_me(x:ProfileIn,u:User=Depends(current_user),s:Session=Depends(db)):
    other=s.scalar(select(User).where(User.phone==x.phone,User.id!=u.id))
...
def create_payment(x:PaymentIn,u:User=Depends(current_user),s:Session=Depends(db)):
    ...
    if x.method not in {"multicaixa_express","gpo"}: raise HTTPException(400,"Método inválido.")
    commission=round(o.total*COMMISSION_RATE,2); net=round(o.total-commission,2)
    reference=f"AGL-{o.id}-{secrets.token_hex(5).upper()}"
    p=Payment(order_id=o.id,method=x.method,reference=reference,amount=o.total,commission_amount=commission,net_to_seller=net,status="aguardando")
    s.add(p); o.status="pagamento_pendente"; s.commit(); s.refresh(p)
    return {"id":p.id,"status":p.status,"amount":p.amount,"commission_amount":p.commission_amount,"commission_rate":COMMISSION_RATE,
            "net_to_seller":p.net_to_seller,"reference":p.reference,"message":"Pedido de pagamento preparado. A cobrança real depende das credenciais/contrato do gateway EMIS/MULTICAIXA do comerciante."}
...
def confirm_payment(payment_id:int,u:User=Depends(current_user),s:Session=Depends(db)):
    ...
    p.status="pago"; o=s.get(Order,p.order_id); o.status="pago"
    notify(s,o.buyer_id,"Pagamento confirmado",f"O pagamento do pedido #{o.id} foi confirmado.","payment")
    notify(s,o.seller_id,"Pagamento confirmado",f"O pagamento do pedido #{o.id} foi confirmado.","payment")
    if o.driver_id: notify(s,o.driver_id,"Pagamento confirmado",f"O pagamento do pedido #{o.id} foi confirmado.","payment")
...
def admin_overview(u:User=Depends(current_user), s:Session=Depends(db)):
    ...
    return {"summary":admin_summary(u,s),
            ...
            "payments":[{"id":p.id,"order_id":p.order_id,"method":p.method,"reference":p.reference,"amount":p.amount,"commission_amount":p.commission_amount,"net_to_seller":p.net_to_seller,"status":p.status,"gateway_reference":p.gateway_reference} for p in payments],
