...
@app.get("/health")
def health(): return {"status":"ok","app":"AgroLink Angola","version":"7.0.0","commission_rate":COMMISSION_RATE,"database":DATABASE_URL.split(":",1)[0],"tables":inspect(engine).get_table_names()}
...
def register(x:Register,s:Session=Depends(db)):
    ...
    u=User(name=x.name,phone=x.phone,password_hash=hash_password(x.password),role=x.role,province=x.province,
