...
Versão integrada para testes comerciais do AgroLink Angola v10.
...
A área Admin só aparece depois de autenticar uma conta com `role=admin`. Para criar o administrador em um ambiente de deploy, configure no backend/Render `ADMIN_PHONE` e `ADMIN_PASSWORD` (mínimo 8 caracteres). Opcionalmente use `ADMIN_NAME`, `ADMIN_PROVINCE`, `ADMIN_COMPANY` e `ADMIN_ADDRESS`. As credenciais não devem ser colocadas no frontend.
