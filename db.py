...
def init_db():
    ...
    for n in ["Cereais","Hortícolas","Frutas","Tubérculos","Leguminosas","Pecuária","Avicultura","Outros"]: c.execute("INSERT OR IGNORE INTO categories(name) VALUES(?)",(n,))
    # Compatibility migration for older v11 databases.
    cols={r[1] for r in c.execute("PRAGMA table_info(orders)").fetchall()}
    if "transport_mode" not in cols: c.execute("ALTER TABLE orders ADD COLUMN transport_mode TEXT NOT NULL DEFAULT 'pending'")
    if "delivery_distance_km" not in cols: c.execute("ALTER TABLE orders ADD COLUMN delivery_distance_km REAL DEFAULT 0")
    if "delivery_fee_kz" not in cols: c.execute("ALTER TABLE orders ADD COLUMN delivery_fee_kz REAL DEFAULT 0")
    if "delivery_status" not in cols: c.execute("ALTER TABLE orders ADD COLUMN delivery_status TEXT DEFAULT 'NOT_REQUESTED'")
    c.commit(); c.close()
