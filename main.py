...
app=FastAPI(title=APP_NAME,version="12.0.0")
TOKENS={}
...
def register(x:Register):
    ...
    if x.role=="seller" and not (x.company_name or x.farm_name): db.close(); raise HTTPException(422,"Indique empresa ou fazenda")
    if x.role=="transporter" and not x.bi_number: db.close(); raise HTTPException(422,"BI é obrigatório para transportador")
    # Store a blind HMAC for lookup; the current SQLite MVP does not implement reversible AES encryption.
    bih=blind_hash(x.bi_number) if x.bi_number else None
    bie="PROTECTED:"+hashlib.sha256((x.bi_number or "").encode()).hexdigest() if x.bi_number else None
...
def me(u=Depends(auth)):
    db=get_db(); p=db.execute("SELECT * FROM producers WHERE user_id=?",(u["id"],)).fetchone(); v=db.execute("SELECT * FROM vehicles WHERE transporter_id=?",(u["id"],)).fetchall(); db.close()
    out=dict(u); out.pop("password_hash",None); out["producer"]=dict(p) if p else None; out["vehicles"]=[dict(x) for x in v]; return out
...
def create_order(x:Order,u=Depends(role("buyer"))):
    ...
    total=round(x.quantity*p["price_kz"],2); c=db.execute("INSERT INTO orders(buyer_id,seller_id,product_id,quantity,delivery_address,transport_mode,product_total_kz,total_kz) VALUES(?,?,?,?,?,?,?,?,?)",(u["id"],p["seller_id"],p["id"],x.quantity,x.delivery_address,"pending",total,total)); oid=c.lastrowid
    db.execute("INSERT INTO conversations(order_id) VALUES(?)",(oid,)); notify(db,p["seller_id"],"Novo pedido",f"Pedido #{oid} aguarda análise."); audit(db,u["id"],"ORDER_CREATED","orders",oid); db.commit(); db.close(); return {"order_id":oid,"status":"PENDING_SELLER"}
...
def available_transporters(u=Depends(auth)):
    if u["role"] not in ("buyer","seller","admin"): raise HTTPException(403,"Sem acesso")
    db=get_db(); rows=[dict(x) for x in db.execute("""SELECT u.id transporter_id,u.full_name,v.id vehicle_id,v.plate,v.model,v.capacity_kg,v.vehicle_type,v.photo,v.status,v.latitude,v.longitude,v.last_ping_at FROM users u JOIN vehicles v ON v.transporter_id=u.id WHERE u.role='transporter' AND u.status='active' AND v.status='FREE' ORDER BY u.full_name""").fetchall()]; db.close(); return rows
...
def save_vehicle(x:Vehicle,u=Depends(role("transporter"))):
    ...
    c=db.execute("INSERT INTO vehicles(transporter_id,plate,model,capacity_kg,vehicle_type,photo,status) VALUES(?,?,?,?,?,?,?)",(u["id"],x.plate.strip().upper(),x.model,x.capacity_kg,x.vehicle_type,x.photo,"FREE")); audit(db,u["id"],"VEHICLE_CREATED","vehicles",c.lastrowid); db.commit(); db.close(); return {"id":c.lastrowid}
...
def accept_delivery(did:int,u=Depends(role("transporter"))):
...
def reject_delivery(did:int,u=Depends(role("transporter"))):
    db=get_db(); d=db.execute("SELECT * FROM deliveries WHERE id=? AND transporter_id=?",(did,u["id"])).fetchone()
    if not d: db.close(); raise HTTPException(404,"Solicitação não encontrada")
...
def delivery_status(did:int,status:str,u=Depends(role("transporter"))):
...
def admin_transporters(u=Depends(role("admin"))):
    db=get_db(); rows=[dict(x) for x in db.execute("SELECT u.id,u.full_name,u.phone,u.province,u.status,v.id vehicle_id,v.plate,v.model,v.capacity_kg,v.status vehicle_status FROM users u LEFT JOIN vehicles v ON v.transporter_id=u.id WHERE u.role='transporter' ORDER BY u.id DESC").fetchall()]; db.close(); return rows
