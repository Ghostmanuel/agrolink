...
app=FastAPI(title=APP_NAME,version="16.1.0")
TOKENS={}
...
def delivery_quote(oid:int,x:DeliveryQuote,u=Depends(auth)):
    ...
    fee=calc_fee(distance_km); total=recalc_total(db,oid,distance_km,fee); db.execute("UPDATE orders SET delivery_address=? WHERE id=?",(destination_text,oid))
    ...
    return {"distance_km":distance_km,"eta_minutes":eta,"delivery_fee_kz":fee,"product_total_kz":o["product_total_kz"],"total_kz":total,"source":source,"origin":origin,"destination":destination}
...
def chat_list(oid:int,u=Depends(auth)):
    ...
    rows=[] if not c else [dict(x) for x in db.execute("SELECT m.*,u.full_name sender_name FROM chat_messages m JOIN users u ON u.id=m.sender_id WHERE conversation_id=? ORDER BY m.id",(c["id"],)).fetchall()]
    for r in rows:
